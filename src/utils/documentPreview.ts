...
export function buildReleasePreviewHtml(data: SharedPreviewData) {
  ...
  const rows = data.items
    .map((item) => {
      ...
      const note = cancelled ? 'CANCEL' : item.releaseNote || '';

      return `<tr class="${cancelled ? 'cancelled-item-row' : ''}">
        ...
        <td class="l note-col">${escapeHtml(note)}</td>
      </tr>`;
    })
    .join('');

  const activeItems = data.items.filter((item) => item.status !== 'ST01');
  const totalPallet = activeItems.reduce((sum, item) => sum + (item.pallet ?? 0), 0);
  const totalBox = activeItems.reduce((sum, item) => sum + (item.box ?? 0), 0);
  ...
  const invoicePiece = (group: SharedPreviewData, suffix: string) => {
    const rows = group.items
      .map((item) => {
        ...
        const cancelled = item.status === 'ST01';
        const note = cancelled ? 'CANCEL' : item.invoiceNote || '';

        return `<tr class="invoice-item-row ${cancelled ? 'cancelled-item-row' : ''}">
    ...
    return `<div class="invoice-doc">
      <div class="invoice-title">거래명세표<span>${suffix}</span><span class="invoice-title-issue">발급 No. ${escapeHtml(group.issueNo)}</span></div>
      <table class="invoice-head-table">
        <tr>
          <td class="buyer-cell">
            <table class="inner-table">
              <tr><td colspan="2" class="c invoice-issue-date">${escapeHtml(issueDateFmt)}</td></tr>
...
export function getInvoicePreviewStyles(printMode: boolean) {
  return `body{margin:0;background:${printMode ? '#fff' : '#f3f4f6'};font-family:'Malgun Gothic','Apple SD Gothic Neo',sans-serif;color:#111}.invoice-page{width:210mm;height:297mm;margin:${printMode ? '0' : '0 auto'};background:#fff;box-sizing:border-box;display:flex;flex-direction:column}.invoice-half{height:148.5mm;box-sizing:border-box;padding:6mm 8mm;overflow:hidden}.invoice-half-bottom{border-top:1px dashed #aaa}.invoice-doc{page-break-inside:avoid;break-inside:avoid}.invoice-break{height:10px}.invoice-group-break{height:${printMode ? '0' : '16px'}}.invoice-title{position:relative;text-align:center;font-size:12pt;font-weight:900;margin-bottom:6px;letter-spacing:.1em}.invoice-title span{font-size:9pt;font-weight:400}.invoice-title .invoice-title-issue{position:absolute;right:0;top:50%;transform:translateY(-50%);font-size:8pt;letter-spacing:0;color:#444}.invoice-head-table,.invoice-total-table,.invoice-items-table,.inner-table{width:100%;border-collapse:collapse}.invoice-head-table{border:2px solid #000;margin-bottom:0}.invoice-head-table td{vertical-align:middle;padding:0}.buyer-cell{width:40%;border-right:1px solid #000}.seller-cell{width:60%}.inner-table td{border-bottom:1px solid #000;border-right:1px solid #000;padding:2px 3px;font-size:9pt;vertical-align:middle}.inner-table tr:last-child td{border-bottom:0}.inner-table td:last-child{border-right:0}.inner-table .narrow{width:28px}.inner-table .label{width:48px}.inner-table .strong{font-weight:700}.inner-table .vertical{width:20px;text-align:center;line-height:1.4;vertical-align:middle}.inner-table .c{text-align:center}.inner-table .invoice-issue-date{font-size:10pt;font-weight:700}.invoice-total-table{border:2px solid #000;border-top:0}.invoice-total-table td{padding:4px 8px;font-size:9pt;font-weight:700;vertical-align:middle}.invoice-total-table .label-cell{width:20%;border-right:1px solid #000}.invoice-total-table .value-cell{text-align:right}.invoice-items-table{border:2px solid #000;border-top:0;text-align:center;table-layout:fixed}.invoice-items-table th,.invoice-items-table td{border-right:1px solid #000;border-bottom:1px solid #000;padding:4px 2px;font-size:9pt;vertical-align:middle}.invoice-items-table th:last-child,.invoice-items-table td:last-child{border-right:0}.invoice-items-table tbody tr:last-child td{border-bottom:0}.invoice-items-table .l{text-align:left}.invoice-items-table .r{text-align:right}.invoice-items-table .c{text-align:center}.invoice-items-table .invoice-item-row td{font-weight:700}.invoice-items-table .cancelled-item-row td{text-decoration:line-through;color:#6b7280}.invoice-items-table .cancelled-item-row .note-col{text-decoration:none;font-weight:700;color:#111}.invoice-items-table .sum-row td,.invoice-items-table .grand-row td{font-weight:700}.invoice-items-table .date-col{width:62px}.invoice-items-table .product-col{width:auto}.invoice-items-table .qty-col{width:72px}.invoice-items-table .price-col{width:68px}.invoice-items-table .supply-col{width:92px}.invoice-items-table .vat-col{width:72px}.invoice-items-table .note-col{width:92px}.invoice-note-area{margin-top:6px;font-size:9pt;line-height:1.4;text-align:left;padding:0 3px}@media print{@page{margin:0;size:A4 portrait}body{background:#fff}.invoice-page{margin:0}.invoice-group-break{page-break-after:always;break-after:page}}`;
}
